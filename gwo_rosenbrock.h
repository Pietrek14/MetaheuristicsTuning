...
namespace gwo
{
	inline void run_rosenbrock_experiments()
	{
		...
		logging::csv_writer writer("gwo_results_rosenbrock", headers);
		...
		run_hyperparam_set_parallel<
			grey_wolf_optimization<T, 3>, hyperparams<T, 3>>(
				rosenbrock3, num_trials, top_percent, writer, write_mutex);
		run_hyperparam_set_parallel<
			grey_wolf_optimization<T, 6>, hyperparams<T, 6>>(
				rosenbrock6, num_trials, top_percent, writer, write_mutex);
		run_hyperparam_set_parallel<
			grey_wolf_optimization<T, 9>, hyperparams<T, 9>>(
