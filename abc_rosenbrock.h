...
namespace abc
{
	inline void run_rosenbrock_experiments()
	{
		...
		logging::csv_writer writer("abc_results_rosenbrock", headers);
		std::mutex write_mutex;


		auto rosenbrock3 = generate_hyperparams<T, 3>(
			core::make<T, 3>("rosenbrock"), 50, 75);

		auto rosenbrock6 = generate_hyperparams<T, 6>(
			core::make<T, 6>("rosenbrock"), 150, 225);

		auto rosenbrock9 = generate_hyperparams<T, 9>(
			core::make<T, 9>("rosenbrock"), 300, 450);
