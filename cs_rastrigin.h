...
namespace cs
{
	inline void run_rastrigin_experiments()
	{
		...
		logging::csv_writer writer("cs_results_rastrigin", headers);
		...
		run_hyperparam_set_parallel<cuckoo_search<T, 3>, hyperparams<T, 3>>(
				rastrigin3, num_trials, top_percent, writer, write_mutex);
		run_hyperparam_set_parallel<cuckoo_search<T, 6>, hyperparams<T, 6>>(
				rastrigin6, num_trials, top_percent, writer, write_mutex);
		run_hyperparam_set_parallel<cuckoo_search<T, 9>, hyperparams<T, 9>>(
